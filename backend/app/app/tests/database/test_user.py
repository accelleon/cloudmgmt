...
def test_password_auth(db: Session) -> None:
    ...
    user = database.user.create(db, obj_in=user_in)
    auth_user = database.user.authenticate_password(db, username=username, password=password)
    assert auth_user
...
def test_update_user(db: Session) -> None:
    ...
    database.user.update(db, obj_in=user, update_data=user_in_update)
    user2 = database.user.get(db, id=user.id)
    assert user2
    assert user.username == user2.username
    assert verify_password(new_password, user2.password)
    assert user2.first_name == user2.last_name == name
...
def test_enable_twofa(db: Session) -> None:
    ...
    user = database.user.update(db, obj_in=user, update_data=update_data)
    ...
    user2 = database.user.update(db, obj_in=user, update_data=update_data)   
